...
import {
  LayoutDashboard, Users, UserCheck, DollarSign, BarChart3, Shield, FileText, Settings, LogOut, Lock
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="ml-2 text-[10px] bg-red-500/20 text-red-300 px-2 py-0.5 rounded-full border border-red-500/30">ADMIN PANEL</span>
            </div>
          </div>
          <div className="flex items-center gap-3 text-sm">
            <span className="text-slate-400 text-xs hidden sm:block">{user?.full_name}</span>
            <Link to="/" className="text-xs text-slate-400 hover:text-slate-200 px-2 py-1 rounded border border-slate-700 hover:border-slate-600">
              View Site
            </Link>
            <button onClick={handleSignOut} className="p-1.5 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white" title="Logout">
            ...
            <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden sticky top-20">
              <div className="p-4 border-b border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-semibold text-white text-sm">{user?.full_name || 'Admin'}</div>
              ...
              <nav className="p-2 text-sm">
                {adminSidebar.map(item => (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl mb-0.5 transition ${
