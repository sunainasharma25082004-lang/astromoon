      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-gray-600">Authentic, energized spiritual products</p>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {products.map((product, index) => (
            <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} whileHover={{ y: -5 }} className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden group">
              <div className="relative">
                <img src={product.image} alt={product.name} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
                {product.original_price && <span className="absolute top-3 left-3 px-2 py-1 bg-red-500 text-white text-xs rounded-md">{Math.round((1 - product.price / product.original_price) * 100)}% OFF</span>}
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <button onClick={() => handleQuickAdd(product)} className="px-3 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm rounded-lg">Add</button>
                  </div>
                </div>
              </div>
            </motion.div>
