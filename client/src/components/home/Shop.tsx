...
export function ShopSection() {
  ...
  useEffect(() => {
    apiFetch('/products')
      .then((data: any) => setProducts(withIds(data).slice(0, 4)))
      .catch(() => setProducts([]));
  }, []);
  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4">
          ...
          {products.map((product, index) => (
            <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} whileHover={{ y: -5 }} className="bg-white rounded-2xl shadow-lg border overflow-hidden group">
              <div className="relative">
                <img src={product.images?.[0] || FALLBACK_IMAGE} alt={product.name} className="w-full h-48 object-cover group-hover:scale-105 transition-transform" />
                {product.original_price && product.original_price > product.price && (
                  <span className="absolute top-3 left-3 px-2 py-1 bg-red-500 text-white text-xs rounded">
                    {Math.round((1 - product.price / product.original_price) * 100)}% OFF
                  </span>
                )}
                <button type="button" className="absolute top-3 right-3 w-8 h-8 bg-white/80 rounded-full flex items-center justify-center"><Heart className="w-4 h-4 text-gray-400" /></button>
              </div>
              <div className="p-4">
                ...
                <div className="flex items-center mb-3">
                  ...
                  <span className="text-sm text-gray-600 ml-1">{product.rating} ({product.total_reviews || 0})</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-lg font-bold">{formatCurrency(product.price)}</span>
                    {product.original_price && <span className="text-sm text-gray-400 line-through ml-2">{formatCurrency(product.original_price)}</span>}
                  </div>
                  <div className="flex gap-1.5">
                    <Link to={`/shop/product/${product.id}`} className="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-sm rounded-lg">View</Link>
                    <button type="button" onClick={() => handleQuickAdd(product)} className="px-3 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm rounded-lg">Add</button>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {products.length === 0 && (
          <p className="text-center text-gray-500 text-sm">Shop products loading… Run <code className="bg-gray-200 px-1 rounded">npm run seed</code> on backend if empty.</p>
        )}
      </div>
    </section>
  );
